...
| Command | Does |
|---|---|
| `STATUS` / `WHOAMI` | addr, component, tapeZero, pitch, angle/target, magnet health, i2cErrors (+ `invertA`/`invertB` on alpha02, or `[RS485ECHO ON]` banner on alpha01 if active) |
...
| `STOP` | brake motor A immediately |
...
## RS485 bus commands (not Modbus — placeholder frame, see project.md)

Frame: `[0xAA][ADDR][CMD][LEN][PAYLOAD...][CRC8]`. `ADDR`: `0x00` broadcast
(also "still unassigned"), `1`–`247` unicast.

| Command | Code | Payload → | Reply |
|---|---|---|---|
| `CMD_PING` | `0x01` | — | `CMD_PONG` (`0x81`) |
| `CMD_DISCOVER` | `0x10` | broadcast, — | `CMD_DISCOVER_HERE` (`0x90`): `[nonceHi,nonceLo,componentIdHi,componentIdLo]` |
...
| `CMD_FEED_NEXT` | `0x26` | — | `CMD_ACK`/`CMD_NACK` |
